...
- By default, no Mode is selected—you intentionally choose one each time
  - By default, every chat starts in Ask anything mode (shown as 'Ask'). Switch to Tax Planning or a Custom Mode before you begin; the selected mode applies to that whole chat.
- Modes are available to all eligible users in your firm
  - Ask anything is available to everyone; Custom Modes to all eligible firm users; Tax Planning is a paid add-on (or available during a trial) — without access you'll see an
      'Upgrade' badge and selecting it prompts a request rather than switching.
...
- Produce downloadable files, including PDFs, DOCX, CSV, XLSX
